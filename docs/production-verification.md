...
Cloudflare Pages・Clerk・Render・Neonを通した縦断動作を確認するための手順です。
もとはハッカソン発表前の確認用でしたが、発表は終了しており、
現在は本番環境に手を入れたあとの回帰確認として使います。
...
- **Clerkが開発用キーで動いている。当面このままにします。** コンソールに
  「Clerk has been loaded with development keys」の警告が出ますが、動作には影響しません。
  発表はデモ形式で終了しており、現在は開発者だけが触るため、
  開発インスタンスの制限（ユーザー100人、認証コードの検証が3回／10秒）には届きません。
  不特定多数に公開する段階になったら本番インスタンスへ切り替えます。
...
- **Renderのコールドスタートが遅い。** 一定時間アクセスが無いと
  最初の応答に40秒以上かかりました。動作を確認する前に一度アクセスして起こしておきます。
