...
`DATABASE_URL`の設定状況はこの範囲では確認できていません。
未認証リクエストは認証チェックで止まるため、DBの設定有無にかかわらず401になります。
判定するには認証付きのリクエストが要ります（[API側の確認](#api側の確認)を参照）。
...
9月2日時点で、1・3・7は合格を確認済みです。
2・4〜6・8はClerkのログインが要るため、アカウントを持つ人が実施してください。
